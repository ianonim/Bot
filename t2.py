...
def handle_send_message(message):
    ...
    if len(args) < 3:
        ...
        send_log_to_chat(message, command, "Недостаточно аргументов")
        return
    ...
    if not text_parts:
        ...
        send_log_to_chat(message, command, "Текст не указан")
        return
    ...
    try:
        ...
        send_log_to_chat(message, command, f"Ошибка API: {e.description}")
    except Exception as e:
